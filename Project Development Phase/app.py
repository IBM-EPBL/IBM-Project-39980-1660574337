...
def home():
    if 'user' in session:
        ...
        return render_template('sprint1/home.html', message=username)
    else:
...
@app.route('/register')
def register():
    return render_template('sprint1/registrations/user_registration.html')
...
{url_for('password_reset', token=token, _external=True)}'''
        ...
        return render_template('sprint1/registrations/request_reset_form.html')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user:
                ...
                if reset_link == token:
                    session['email'] = email
                    return render_template('sprint1/registrations/password_reset.html')
                else:
